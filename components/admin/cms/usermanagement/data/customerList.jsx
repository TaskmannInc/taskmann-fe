...
export default function CustomersDataList({
  ...
  var iconSize = 18;
  var totalDataSet = customerUsers?.length;
  return (
    <>
      {isCustomerLoading ? (
        <TableLoader />
      ) : isCustomerSuccess && customerUsers?.length == 0 ? (
        <NoTableData />
      ) : isCustomerSuccess && customerUsers?.length > 0 ? (
        <>
          <table className={styles.dataTable}>
            ...
            <tbody>
              {customerUsers?.map((rowItem, index) => (
                <tr key={index + 1}>
                  <td>{index + 1}</td>
          ...
          {customerUsers?.length > 0 && (
            <div className="pagination-section">
              <div className="pagination-text">
                <p>
                  Showing <strong>{customerUsers?.length ?? 0}</strong> out of{" "}
